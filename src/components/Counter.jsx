  ...
  return (
    <div className="bg-blue-400 text-center p-8 max-w-96  m-auto rounded-2xl mt-8   text-blue-100">
      <div className="flex flex-row gap-5 items-center justify-between">
        <button
          onClick={handleStepSub}
          className="px-2 bg-blue-500 text-white text-lg"
        >
          -
        </button>
        <p>Step: {step}</p>
      ...
      {count >= 1 && (
        <div className="mb-4">
          ...
          <p className="mt-6">
            The {timeDirection} date {Math.abs(count)} days from today is{" "}
            {formattedFutureDate}.
          </p>
        </div>
      )}
      ...
      <div className="py-5">
        <button
          onClick={handleReset}
          className=" m-auto bg-white text-blue-900 p-4 rounded-2xl"
        >
          Reset
        </button>
      </div>
